...
module.exports = class WyzeMotionSensor extends WyzeAccessory {
  ...
  getSensorService () {
    if(this.plugin.config.logLevel == "debug") this.plugin.log(`[MotionSensor] Retrieving previous service for "${this.display_name}"`)
    let service = this.homeKitAccessory.getService(HOMEBRIDGE_SERVICE)

    if (!service) {
      if(this.plugin.config.logLevel == "debug") this.plugin.log(`[MotionSensor] Adding service for "${this.display_name}"`)
  ...
  getBatterySensorService () {
    if(this.plugin.config.logLevel == "debug") this.plugin.log(`[MotionSensorBattery] Retrieving previous service for "${this.display_name}"`)
    let service = this.homeKitAccessory.getService(HOMEBRIDGE_BATTERY_SERVICE)

    if (!service) {
      if(this.plugin.config.logLevel == "debug") this.plugin.log(`[MotionSensorBattery] Adding service for "${this.display_name}"`)
  ...
  getIsBatteryLowSensorService () {
    if(this.plugin.config.logLevel == "debug") this.plugin.log(`[MotionSensorIsBatteryLow] Retrieving previous service for "${this.display_name}"`)
    let service = this.homeKitAccessory.getService(HOMEBRIDGE_BATTERY_SERVICE)

    if (!service) {
      if(this.plugin.config.logLevel == "debug") this.plugin.log(`[MotionSensorIsBatteryLow] Adding service for "${this.display_name}"`)
      service = this.homeKitAccessory.addService(HOMEBRIDGE_BATTERY_SERVICE)
    }
  ...
  getOnCharacteristic () {
    if(this.plugin.config.logLevel == "debug") this.plugin.log(`[MotionSensor] Fetching status of "${this.display_name}"`)
    return this.getSensorService().getCharacteristic(HOMEBRIDGE_CHARACTERISTIC)
  }

  getBatteryCharacteristic () {
    if(this.plugin.config.logLevel == "debug") this.plugin.log(`[MotionSensorBattery] Fetching status of "${this.display_name}"`)
  ...
  updateCharacteristics (device) {
    if(this.plugin.config.logLevel == "debug") this.plugin.log(`[MotionSensor] Updating status of "${this.display_name}"`)
