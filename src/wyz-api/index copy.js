...
module.exports = class WyzeAPI {
  ...
  async _sendSmsCode (sessionId, userId) {
    const url = 'https://auth-prod.api.wyze.com/user/login/sendSmsCode'
   ...
   //this.sms_session_id = result.session_id
   //console.log(this.sms_session_id)
  } catch (error) {
    console.log(error)
  ...
  async login () {
    ...
    console.log(result.data.mfa_options[0])
    // Do we need to perform a 2-factor login?
    if (result.data.mfa_options[1] == 'TotpVerificationCode') {
      console.log('Login first time, Totp enable')

      this.two_factor_type = 'TOTP'
      this.verification_id = result.data.mfa_details.totp_apps[0].app_id
      this.getTokenWith2fa()
    } 
    // else if (result.data.mfa_options[0] == 'PrimaryPhone' || result.data.mfa_options[1] == 'PrimaryPhone') {
    //   console.log('Login first time, SMS enable')

    //   this.two_factor_type = 'SMS'

    //   console.log(result.data.sms_session_id)
    //   console.log(result.data.user_id)
    //   await this._sendSmsCode(result.data.sms_session_id, result.data.user_id)
  ...
  async getTokenWith2fa () {
    if (this.two_factor_type == "TOTP") {
      ...
      if ( result.description == 'Invalid Verification Code.') {
        throw new Error('Your account has 2-factor auth enabled. The provided "mfaCode" parameter in config.json has expired.')
      }
    } else {
      if (!this.smsCode) {
        throw new Error('Your account has 2-factor auth enabled. Please provide the "smsCode" parameter in config.json.')
      }
      const data = {
      ...
      if ( result.description == 'Invalid Verification Code.') {
        throw new Error('Your account has 2-factor auth enabled. The provided "smsCode" parameter in config.json has expired.')
      }
    }
  ...
  async _updateTokens ({ access_token, refresh_token, session_id, user_id }) {
