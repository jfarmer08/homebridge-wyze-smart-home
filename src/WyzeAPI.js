...
module.exports = class WyzeAPI {
  ...
  async runActionList (deviceMac, deviceModel, propertyId, propertyValue) {
    ...
    if (propertyId !== WyzeConstants.WYZE_API_POWER_PROPERTY) {
      plist.push({
        pid: 'P3',
        pvalue: '1'
      })
  ...
  async controlLock (deviceMac, deviceModel, action) {
    ...
    body.sign = md5(encodeURIComponent(`post${WyzeConstants.WYZE_LOCK_CONTROL_URL}${Object.keys(body).sort().map(key => `${key}=${body[key]}`).join('&')}${this.appSecret}`))
